...
class ShardingDropIndexSupportedCheckerTest {
    ...
    void assertCheckWhenIndexExistForPostgreSQL() {
        ...
        // SPEX CHANGED: BEGIN
        assertDoesNotThrow(() -> new ShardingDropIndexSupportedChecker().check(rule, mock(), database, schema, new DropIndexStatementContext(sqlStatement)));
        // SPEX CHANGED: END
    }
    ...
    void assertCheckWhenIndexNotExistForPostgreSQL() {
        ...
        // SPEX CHANGED: BEGIN
        assertThrows(IndexNotFoundException.class, () -> new ShardingDropIndexSupportedChecker().check(rule, mock(), database, mock(), new DropIndexStatementContext(sqlStatement)));
        // SPEX CHANGED: END
    }
}
