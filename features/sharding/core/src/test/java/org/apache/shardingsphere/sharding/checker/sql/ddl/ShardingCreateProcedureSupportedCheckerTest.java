...
class ShardingCreateProcedureSupportedCheckerTest {
    ...
    void assertCheckForMySQL() {
        ...
        // SPEX CHANGED: BEGIN
        CreateProcedureStatementContext sqlStatementContext = new CreateProcedureStatementContext(sqlStatement, "foo_db", mock(ShardingSphereMetaData.class));
        assertDoesNotThrow(() -> new ShardingCreateProcedureSupportedChecker().check(rule, mock(ShardingSphereMetaData.class), database, schema, sqlStatementContext));
        // SPEX CHANGED: END
    }
    ...
    void assertCheckWithShardingTableForMySQL() {
        ...
        assertThrows(UnsupportedShardingOperationException.class,
                () -> new ShardingCreateProcedureSupportedChecker().check(rule, mock(ShardingSphereMetaData.class), database, mock(), sqlStatementContext));
        // SPEX CHANGED: END
    }
    ...
    void assertCheckWithTableExistsForMySQL() {
        ...
        assertThrows(TableExistsException.class, () -> new ShardingCreateProcedureSupportedChecker().check(rule, mock(ShardingSphereMetaData.class), database, schema, sqlStatementContext));
        // SPEX CHANGED: END
    }
}
