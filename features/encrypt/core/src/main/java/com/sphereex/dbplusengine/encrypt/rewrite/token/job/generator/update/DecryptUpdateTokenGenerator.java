...
/**
 * Update token generator for decrypt.
 */
@HighFrequencyInvocation
@RequiredArgsConstructor
@Setter
public final class DecryptUpdateTokenGenerator implements CollectionSQLTokenGenerator<UpdateStatementContext>, HintValueContextAware {
    ...
    private Collection<SQLToken> generateAssignmentSQLTokens(final UpdateStatementContext sqlStatementContext, final EncryptTable table) {
        ...
        for (ColumnAssignmentSegment each : assignmentSegment.get().getAssignments()) {
            ...
            Optional<PlainColumnItem> plainColumn = table.getEncryptColumn(columnSegment.getIdentifier().getValue()).getPlain();
            if (plainColumn.isPresent()) {
                QuoteCharacter quoteCharacter = columnSegment.getIdentifier().getQuoteCharacter();
                Projection projection = new ColumnProjection(
                        null, new IdentifierValue(plainColumn.get().getName(), quoteCharacter), null, sqlStatementContext.getDatabaseType());
                result.add(new SubstitutableColumnNameToken(startIndex, stopIndex, Collections.singleton(projection), sqlStatementContext.getDatabaseType(), database, metaData));
                result.add(getDecryptJobUpdateTokenGenerator(sqlStatementContext, plainColumn.get(), quoteCharacter));
            }
        }
        return result;
    }
    
    private NoneUniqueKeyScenarioUpdateColumnToken getDecryptJobUpdateTokenGenerator(final UpdateStatementContext statementContext, final PlainColumnItem plainColumn,
                                                                                     final QuoteCharacter quoteCharacter) {
        WhereSegment whereSegment = statementContext.getWhereSegments().iterator().next();
        return new NoneUniqueKeyScenarioUpdateColumnToken(whereSegment.getStopIndex() + 1, whereSegment.getStopIndex(), plainColumn.getName(), quoteCharacter);
    }
}
