...
/**
 * Encrypt max function encrypt SQL token generator.
 */
@HighFrequencyInvocation
public final class EncryptMaxFunctionSQLTokenGenerator implements EncryptFunctionSQLTokenGenerator {
    ...
    public Collection<SQLToken> generateSQLTokens(final EncryptRule encryptRule, final Map<String, EncryptRule> databaseEncryptRules,
        ...
        return Collections.singleton(
                new SubstitutableColumnNameToken(columnSegment.getStartIndex(), columnSegment.getStopIndex(), createColumnProjections(encryptColumn.getOrderQuery().map(OrderQueryColumnItem::getName)
                        .orElseGet(() -> encryptColumn.getCipher().getName()), columnSegment.getIdentifier().getQuoteCharacter(), databaseType), databaseType, database, metaData));
    }
