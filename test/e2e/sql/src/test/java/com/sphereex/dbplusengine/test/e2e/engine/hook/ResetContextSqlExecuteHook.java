...
public class ResetContextSqlExecuteHook implements SqlExecuteHook {
    
    private String preCaseSuite;
    
    @Override
    public boolean test(final String caseId, final String sql, final ComparisonDataSourceContext context) {
        String caseSuite = StringUtils.substringBefore(caseId, ":");
        if (preCaseSuite != null && !preCaseSuite.equals(caseSuite)) {
            context.reset();
        }
        preCaseSuite = caseSuite;
        return true;
    }
}
