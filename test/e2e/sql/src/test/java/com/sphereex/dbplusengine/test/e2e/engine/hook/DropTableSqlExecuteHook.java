...
public class DropTableSqlExecuteHook extends AbstratParserSqlExecuteHook {
    ...
    public void after(final String sql, final ComparisonDataSourceContext context) {
        ...
        List<String> tables = ((DropTableStatement) sqlStatement).getTables().stream().map(SimpleTableSegment::getTableName).map(TableNameSegment::getIdentifier)
                .map(IdentifierValue::getValue).collect(Collectors.toList());
        context.removeEncryptTableRule(tables);
    }
}
