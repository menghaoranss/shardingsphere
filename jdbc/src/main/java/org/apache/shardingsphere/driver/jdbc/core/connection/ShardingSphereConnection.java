...
import com.sphereex.dbplusengine.infra.exception.kernel.syntax.hint.DataSourceHintNotAllowedException;
...
public final class ShardingSphereConnection extends AbstractConnectionAdapter {
    ...
    public PreparedStatement prepareStatement(final String sql) throws SQLException {
        // SPEX ADDED: BEGIN
        if (isUseNativeStatement(sql)) {
            return getNativeConnection().prepareStatement(sql);
        }
        // SPEX ADDED: END
    ...
    public PreparedStatement prepareStatement(final String sql, final int resultSetType, final int resultSetConcurrency) throws SQLException {
        // SPEX ADDED: BEGIN
        if (isUseNativeStatement(sql)) {
            return getNativeConnection().prepareStatement(sql, resultSetType, resultSetConcurrency);
        }
        // SPEX ADDED: END
    ...
    public PreparedStatement prepareStatement(final String sql, final int resultSetType, final int resultSetConcurrency, final int resultSetHoldability) throws SQLException {
        // SPEX ADDED: BEGIN
        if (isUseNativeStatement(sql)) {
            return getNativeConnection().prepareStatement(sql, resultSetType, resultSetConcurrency, resultSetHoldability);
        }
        // SPEX ADDED: END
    ...
    public PreparedStatement prepareStatement(final String sql, final int autoGeneratedKeys) throws SQLException {
        // SPEX ADDED: BEGIN
        if (isUseNativeStatement(sql)) {
            return getNativeConnection().prepareStatement(sql, autoGeneratedKeys);
        }
        // SPEX ADDED: END
    ...
    public PreparedStatement prepareStatement(final String sql, final int[] columnIndexes) throws SQLException {
        // SPEX ADDED: BEGIN
        if (isUseNativeStatement(sql)) {
            return getNativeConnection().prepareStatement(sql, columnIndexes);
        }
        // SPEX ADDED: END
    ...
    public PreparedStatement prepareStatement(final String sql, final String[] columnNames) throws SQLException {
        // SPEX ADDED: BEGIN
        if (isUseNativeStatement(sql)) {
            return getNativeConnection().prepareStatement(sql, columnNames);
        }
        // SPEX ADDED: END
    ...
    @SphereEx
    private boolean isUseNativeStatement(final String sql) {
        return isOnlyOneDataSource() && (isOnlyOneEncryptRule() || isOnlyOneShardingRule()) && isSQLCommentTagExist(sql);
    }
    
    @SphereEx
    private boolean isOnlyOneDataSource() {
        return 1 == getCurrentDatabase().getResourceMetaData().getDataSources().size();
    }
    
    @SphereEx
    private boolean isOnlyOneEncryptRule() {
        ShardingSphereDatabase currentDatabase = getCurrentDatabase();
        // NOTE: 新版本默认包含 SingleRule，因此判断 2 == size
        return 2 == currentDatabase.getRuleMetaData().getRules().size() && currentDatabase.getRuleMetaData().findSingleRule(EncryptRule.class).isPresent();
    }
    
    @SphereEx
    private boolean isOnlyOneShardingRule() {
        ShardingSphereDatabase currentDatabase = getCurrentDatabase();
        // NOTE: 新版本默认包含 SingleRule，因此判断 2 == size
        return 2 == currentDatabase.getRuleMetaData().getRules().size() && currentDatabase.getRuleMetaData().findSingleRule(ShardingRule.class).isPresent();
    }
    
    @SphereEx
    private boolean isSQLCommentTagExist(final String sql) {
        Optional<String> sqlComment = findSQLComment(sql);
        return sqlComment.map(optional -> optional.contains("-VENUS-NO-SS-")).orElse(false);
    }
    
    @SphereEx
    private Optional<String> findSQLComment(final String sql) {
        if (Strings.isNullOrEmpty(sql)) {
            return Optional.empty();
        }
        int beginIndex = sql.indexOf("/*");
        int endIndex = sql.indexOf("*/");
        return beginIndex >= 0 && endIndex > beginIndex ? Optional.of(sql.substring(beginIndex, endIndex + 2)) : Optional.empty();
    }
    
    @SphereEx
    private Connection getNativeConnection() throws SQLException {
        Map<String, DataSource> dataSources = getCurrentDatabase().getResourceMetaData().getDataSourceMap();
        ShardingSpherePreconditions.checkState(1 == dataSources.size(), () -> new DataSourceHintNotAllowedException(Joiner.on(", ").join(dataSources.keySet())));
        return databaseConnectionManager.getRandomConnection();
    }
    
    @SphereEx
    private ShardingSphereDatabase getCurrentDatabase() {
        ShardingSphereMetaData metaData = contextManager.getMetaDataContexts().getMetaData();
        ShardingSpherePreconditions.checkState(metaData.containsDatabase(currentDatabaseName), () -> new UnknownDatabaseException(currentDatabaseName));
        return metaData.getDatabase(currentDatabaseName);
    }
}
