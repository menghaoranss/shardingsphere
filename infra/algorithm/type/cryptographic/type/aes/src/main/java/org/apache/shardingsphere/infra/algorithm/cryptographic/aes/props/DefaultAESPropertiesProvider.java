...
public final class DefaultAESPropertiesProvider implements CryptographicPropertiesProvider {
    ...
    @SphereEx
    private static final Collection<Integer> CANDIDATE_AES_KEY_BIT_LENGTH = new HashSet<>(Arrays.asList(DEFAULT_AES_KEY_BIT_LENGTH, 192, 256));
    ...
    private byte[] getSecretKey(final Properties props) {
        ...
        // SPEX CHANGED: BEGIN
        int keyBitLength = Integer.parseInt(props.getProperty(AES_KEY_BIT_LENGTH, String.valueOf(DEFAULT_AES_KEY_BIT_LENGTH)));
        ShardingSpherePreconditions.checkContains(CANDIDATE_AES_KEY_BIT_LENGTH, keyBitLength,
                () -> new AlgorithmInitializationException(this, "%s should in %s", AES_KEY_BIT_LENGTH, CANDIDATE_AES_KEY_BIT_LENGTH));
        return Arrays.copyOf(DigestUtils.getDigest(digestAlgorithm.toUpperCase()).digest(aesKey.getBytes(StandardCharsets.UTF_8)), keyBitLength / 8);
        // SPEX CHANGED: END
    }
