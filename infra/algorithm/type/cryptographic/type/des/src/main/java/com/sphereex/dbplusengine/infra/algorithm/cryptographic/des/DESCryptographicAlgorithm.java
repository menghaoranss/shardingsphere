 ...
 */
public final class DESCryptographicAlgorithm implements CryptographicAlgorithm {
    
    private static final String DES_KEY = "des-key-value";
    
    private static final String DIGEST_ALGORITHM_NAME = "digest-algorithm-name";
    
    private static final int DES_KEY_BIT_LENGTH = 64;
    ...
    private byte[] getSecretKey(final Properties props) {
        String desKey = props.getProperty(DES_KEY);
        ShardingSpherePreconditions.checkNotEmpty(desKey, () -> new AlgorithmInitializationException(this, "%s can not be null or empty", DES_KEY));
        String digestAlgorithm = props.getProperty(DIGEST_ALGORITHM_NAME, MessageDigestAlgorithms.SHA_256);
        return Arrays.copyOf(DigestUtils.getDigest(digestAlgorithm.toUpperCase()).digest(desKey.getBytes(StandardCharsets.UTF_8)), DES_KEY_BIT_LENGTH / 8);
    }
    ...
    private Cipher getCipher(final int decryptMode) throws GeneralSecurityException {
        ...
        result.init(decryptMode, new SecretKeySpec(secretKey, ALGORITHM_NAME));
        return result;
    }
